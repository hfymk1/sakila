    ...
    <?php
      ...
      if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
		  ...
		  echo "<td> <form method='POST' ><input type='hidden' name='Get_ID' value='" .$row["address_id"] ."'><input type='submit' name='delete' value='Delete'> </form> </td>";
	  ...
	  if (isset($_POST["delete"])){
        $sql = "DELETE FROM address WHERE address_id =" . $_POST["Get_AddID"];


      if ($conn->query($sql) === TRUE) {
        echo'<script type ="text/javascript">';
        echo 'alert("Record deleted successfully.");';
        echo 'window.location="dAddress.php";';
        echo '</script>';
        mysqli_close($conn);
        exit;
    } else {
