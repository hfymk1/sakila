    ...
    <div class="dropdown">
      ...
      <div class="dropdown-content">
				...
				<form method="post" >
					   ...
					   <input type="checkbox" name="Check3" <?php if(isset($_POST['Check3'])) echo "checked='checked'"; ?> /> <font color="white">Description</font><br><br>
					   <input type="submit" value="View">
		        </form>
      </div>
    </div>

    <a href="filmtext.php" class="active"> Back </a>
    ...
    <?php
      ...
      if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
          ...
          $Get_ID =$row["film_id"];
		  echo "<td> <form method='POST' ><input type='hidden' name='Get_ID' value='" .$row["film_id"] ."'><input type='submit' name='delete' value='Delete'> </form> </td>";
	   ...
	   if (isset($_POST["delete"])){
        $sql = "DELETE FROM filmtext WHERE film_id =" . $_POST["Get_ID"];

      if ($conn->query($sql) === TRUE) {
        echo'<script type ="text/javascript">';
        echo 'alert("Record deleted successfully.");';
        echo 'window.location="dFilmtext.php";';
        echo '</script>';
        mysqli_close($conn);
        exit;
    } else {
